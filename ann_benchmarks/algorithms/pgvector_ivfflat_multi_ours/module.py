...
# 尝试导入pgvector，如果失败则跳过
try:
    import pgvector.psycopg
    import psycopg
    PGVECTOR_AVAILABLE = True
except ImportError:
    print("Warning: pgvector not available, using fallback")
...
class PGVectorIVFFlatMulti(BaseANN):
    def __init__(self, metric, method_param):
        ...
        self._cur = None
        self._num_workers = method_param.get('num_workers', 4)  # 默认4个工作线程
    ...
    def ensure_pgvector_extension_created(self, conn) -> None:
        if not PGVECTOR_AVAILABLE:
            print("Warning: pgvector not available, skipping extension creation")
            return
    ...
    def setup_parallel_config(self, conn) -> None:
        """设置PostgreSQL并行配置"""
        if not PGVECTOR_AVAILABLE:
            print("Warning: pgvector not available, skipping parallel config")
            return
